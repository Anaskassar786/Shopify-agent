...
packages/
  ...
  monitoring/ Error-capture port: noop + real Sentry envelope adapter, zero runtime deps
              (launch readiness, ADR 38)
```
...
|---|---|---|
| Types | `pnpm -r run typecheck` | TS strict, zero errors (19 projects) |
| Tests | `pnpm -r run test` | all suites green = 1197 passed + 4 Redis-gated skips (19 projects) |
...
- ✅ **1.1.1 Launch readiness (hardening, not a milestone)** — **ops control plane** (ADR 37: generic `platform_flags` kv + **maintenance mode** — step-up-gated enable/disable/custom-message, typed 503 `MAINTENANCE_MODE` with the operator message, exempt set by construction: health/legal/Shopify webhooks+OAuth/session boot/tracking links/the admin console itself; merchant web renders a calm "We'll be right back" surface — plus **per-merchant feature flags** on the existing `featureOverrides` jsonb: `aiDisabled` gates provider-crossing writes, `automationDisabled` gates workflow mutations AND the worker run-start funnel; reads never gate; both writes fully audited) · **admin console Ops tab** (maintenance toggle + per-store flag editor + durable job-queue readout: queued/running/completed/failed/retries/DLQ, truthful with Redis down) · **error monitoring** (ADR 38: new `@profit/monitoring` — real zero-dependency Sentry envelope adapter + documented no-op absent `SENTRY_DSN`; wired at both process bootstraps + the 5xx error path) · **readiness completed** (`/ready` adds the Shopify app-credentials check — four production signals total) · **`WEBHOOK_SECRET` retired** (ADR 39: dead config from the generic list; Shopify HMAC truth is `SHOPIFY_API_SECRET`) · **runbooks** (`docs/ops/launch-readiness.md` gap map A/B/C · new `backup-recovery-runbook.md` · incident/release runbooks updated) · migration 0009 · +32 tests (1197 passed + 4 Redis-gated skips, 19 projects)
- ◻ M9+ Phase 4–5 — next (public API + partner webhooks, white label, mobile companion)

Migrations: `packages/db/drizzle` (0000 core · 0001 shopify core · 0002 RLS · 0003 data plane · 0004 notifications+onboarding · 0005 AI engine · 0006 billing+growth · 0007 automation+campaigns · 0008 copilot+reports · 0009 launch ops flags).
