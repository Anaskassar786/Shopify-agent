...
3. **AI cost runaway (M×H) — metering + hard cap live at M4.** Pre-aggregated PII-minimized context; one paid call per agent/firing-batch; `AI_MAX_AGENT_CALLS_PER_RUN` circuit breaker (default 5); per-call `ai_call_logs` micro$ cost recording (provider pricing table with conservative fallback); per-store budgets → entitlement enforcement lands with M5 billing gates.
4. **Attribution credibility (M×H) — chain shipped at M4, reporting polish in M5.** Attribution joins by strongest signal first (checkout-token → discount-code containment → customer+window) with per-match conviction recorded; modeled estimates labeled with expectation basis; merchants see methodology text in the UI; before/after cohort ROI reports complete in M5 (P11 risk 2).
