...
6c. **M3 dashboard honesty constraint (process).** P9's profit/AI-score/AI-insights widgets require data that does not exist until M4 (AI) and the cost-fields work (COGS). They are explicitly deferred and labeled in the M3 doc §6 — shipping invented numbers would violate permanent rule 1; the deferred set is small, enumerated, and tracked here so it cannot silently linger: → **profit card (needs COGS source decision by M6), AI score + AI insights (M4), automation widget (M4).**
