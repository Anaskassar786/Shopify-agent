...
37. **Launch readiness: ops control plane = one generic platform kv + one merchant jsonb** — maintenance mode is platform-scope state stored in `platform_flags` (key/value jsonb, one generic row container so future platform flags never need a migration) and toggled ONLY through the step-up-gated admin API with a `platform_admin_actions` row (PRD Part-5 "enable · disable · custom message · admin bypass"). The API 503s merchant data-plane requests with typed `MAINTENANCE_MODE` + the operator message; health/live/ready, `/legal`, `/shopify` (webhooks keep acking into durable intake; OAuth install stays open), `/api/v1/auth` (session boot), `/api/v1/t` (links already in the wild) and `/api/v1/admin` (the bypass) stay up by construction. Per-merchant flags ride the EXISTING `store_settings.featureOverrides` jsonb (taxonomy enum in `@profit/types`, reader in `@profit/db`) — `aiDisabled` gates provider-crossing writes (`POST /recommendations/run`, `POST /copilot/ask`), `automationDisabled` gates workflow mutations AND the worker run-start funnel (schedule/manual/webhook all converge there; in-flight runs finish, documented); reads stay readable — a disabled feature hides no merchant data.
38. **Launch readiness: error capture is a real minimal adapter, never a fake** — `packages/monitoring` holds the `ErrorMonitor` port + `NoopErrorMonitor` + `SentryErrorMonitor` (hand-rolled Sentry envelope POST over fetch — zero new runtime dependencies, DSN-derived ingest URL + `X-Sentry-Auth`, 1.5s timeout, failures swallowed into the pino log; monitoring must never crash the app). Wired at both process bootstraps (`unhandledRejection`, `uncaughtException`) and the express error handler's 500 path. Absent `SENTRY_DSN` ⇒ the no-op monitor and the product still relies on the structured logs (the honest degradation RISKS always promised).
39. **Launch readiness: webhook secret consolidation** — Shopify signs webhook deliveries with the APP SECRET (verified by `verifyWebhookHmac` + M1 replay tests), so `WEBHOOK_SECRET` was dead config from the PRD's generic list: removed from the env schema + `.env.example`. One HMAC source of truth (`SHOPIFY_API_SECRET`), no unused secret lying around to drift.
